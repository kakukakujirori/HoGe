# https://github.com/microsoft/MoGe/blob/main/moge/utils/geometry_numpy.py
import math
    ...
    if ransac_iters is None:
        ...
        for _ in range(ransac_iters):
            ...
            if confirmed_inliers.sum() > 10:
                solution = least_squares(
                    partial(
                        fn, uv[confirmed_inliers], xy[confirmed_inliers], z[confirmed_inliers]
                    ),
                    x0=maybe_shift,
                    ftol=1e-3,
                    method="lm",
...
def point_map_to_depth_numpy(
    points: np.ndarray, mask: np.ndarray = None, downsample_size: Tuple[int, int] = (64, 64)
):
    import cv2
...
def mask_aware_nearest_resize_numpy(
    ...
    """Resize 2D map by nearest interpolation. Return the nearest neighbor index and mask of the
    resized map.
