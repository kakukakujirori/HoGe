...
class HoGeModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.max_points_per_ray = max_points_per_ray
        ...
        head_dim_out = [3 * max_points_per_ray]  # (points)
        if output_color:
            head_dim_out.append(3 * max_points_per_ray)
        if output_conf:
    ...
    def load_pretrained_moge(self):
        "Load the backbone and the decoder with the trained MoGe weights from torch hub"
        print("[HoGeModel] load_pretrained_moge called")
        cached_checkpoint_path = hf_hub_download(
        ...
        for pname, pval in moge_state_dict.items():
            if pname in model_dict and pval.shape == model_dict[pname].shape:
