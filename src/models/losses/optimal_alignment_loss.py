...
class OptimalAlignmentLoss(torch.nn.Module):
    ...
    def optimal_alignment_solver(
        ...
        input_points = rearrange(
            F.interpolate(
                rearrange(input_points, "b h w c -> b c h w", c=3), (process_size, process_size)
            ),
            "b c h w -> b (h w) c",
            c=3,
        )
        output_points = rearrange(
            F.interpolate(
                rearrange(output_points, "b h w c -> b c h w", c=3), (process_size, process_size)
            ),
            "b c h w -> b (h w) c",
            c=3,
        )
        valid_masks = rearrange(
            F.interpolate(
                rearrange(valid_masks.float(), "b h w -> b () h w"), (process_size, process_size)
            ),
            "b () h w -> b (h w)",
        ).bool()
        ...
        for b in range(batch):
            ...
            Pz = P[:, 2:3]  # (N, 1)
            Pzk_Pzi = Pz.unsqueeze(0) - Pz.unsqueeze(
                1
            )  # (1, N, 1) - (N, 1, 1) -> (Nk, Ni, 1) // Pz_Pz[k, i, :] = Pz[i, :] - Pz[k, :]
            Pxy_Pxy = P[:, :2].unsqueeze(0).expand(N, -1, -1)
            P2 = torch.cat(
                [Pxy_Pxy, Pzk_Pzi], dim=-1
            )  # (N, N, 3) // P2[k, i, :] = [Px^i, Py^i, Pz^i - Pz^k]

            Qz = Q[:, 2:3]  # (N, 1)
            Qzk_Qzi = Qz.unsqueeze(0) - Qz.unsqueeze(
                1
            )  # (1, N, 1) - (N, 1, 1) -> (Nk, Ni, 1) // Qz_Qz[k, i, :] = Qz[i, :] - Qz[k, :]
            Qxy_Qxy = Q[:, :2].unsqueeze(0).expand(N, -1, -1)
            Q2 = torch.cat(
                [Qxy_Qxy, Qzk_Qzi], dim=-1
            )  # (N, N, 3) // Q2[k, i, :] = [Qx^i, Qy^i, Qz^i - Qz^k]
            ...
            ratio_minus_ordered_cumsum = torch.cumsum(
                torch.gather(values, -1, ratio_minus_idx), dim=-1
            )
            ratio_plus_ordered_cumsum = torch.cumsum(
                torch.gather(values, -1, ratio_plus_idx), dim=-1
            )

            # add sentinels for binary search
            zero_padding = torch.tensor([[0]], dtype=values.dtype, device=values.device).expand(
                N, 1
            )
            inf_padding = torch.tensor(
                [[torch.inf]], dtype=values.dtype, device=values.device
            ).expand(N, 1)
            ratio_ordered_cumsum = torch.cat(
                [zero_padding, ratio_ordered_cumsum, inf_padding], dim=-1
            )
            ratio_minus_ordered_cumsum = torch.cat(
                [zero_padding, ratio_minus_ordered_cumsum, inf_padding], dim=-1
            )
            ratio_plus_ordered_cumsum = torch.cat(
                [zero_padding, ratio_plus_ordered_cumsum, inf_padding], dim=-1
            )
            ...
            left_grad = (
                2 * torch.gather(ratio_ordered_cumsum, -1, left_idx)
                - torch.gather(ratio_minus_ordered_cumsum, -1, left_minus_idx)
                - torch.gather(ratio_plus_ordered_cumsum, -1, left_plus_idx)
            )  # (N, N * 3)
            ...
            right_grad = (
                2 * torch.gather(ratio_ordered_cumsum, -1, right_idx)
                - torch.gather(ratio_minus_ordered_cumsum, -1, right_minus_idx)
                - torch.gather(ratio_plus_ordered_cumsum, -1, right_plus_idx)
            )  # (N, N * 3)
            ...
            extrema_mask = (
                torch.logical_and(left_grad < -eps, eps <= right_grad) & ratio.isfinite()
            )  # (N, N * 3)
            ...
            if len(extrema_idx) > 0:
                ...
                cost = torch.sum(
                    torch.abs(P - scale_cand.reshape(-1, 1, 1) * Q - shift_cand.reshape(-1, 1, 3))
                    / torch.abs(Pz),
                    dim=[-2, -1],
                )  # (M, 3) -> (M,)
                ...
                print(
                    f"[optimal_alignment_solver] No solution found. The mask may be empty:\n{torch.sum(mask)=}\n{Pz=}\n{Qz=}"
                )
                try:
                    ...
                    print(e)
                    ret_shift = torch.zeros(
                        3, dtype=input_points.dtype, device=input_points.device
                    )
                    ret_scale = torch.ones(1, dtype=ret_shift.dtype, device=ret_shift.device)[0]
    ...
    def inv_depth_weighted_l1(
        ...
        if masks is not None:
            ...
            if torch.any(masks):
                ...
                loss_per_pixel = (
                    diff / input_points[masks][..., 2:3]
                )  # weighting is inv-proportional to depth
            else:
    ...
    def occluded_region_loss(
        ...
        input_masks_flatten = (
            input_texels_occ[..., 3:].reshape(-1, occ, 1) > 0.9
        )  # NOTE: sky is invalid
        ...
        input_points_len = input_masks_flatten.sum(
            dim=1
        ).flatten()  # NOTE: input_points must be gathered beforehand!!!
        ...
        output_points_fetched = torch.gather(
            output_points_flatten, dim=1, index=idx.expand(-1, -1, 3)
        )  # (b * h * w, occ, 3)
        output_colors_fetched = torch.gather(
            output_colors_flatten, dim=1, index=idx.expand(-1, -1, 3)
        )  # (b * h * w, occ, 3)
        output_confs_fetched = torch.gather(
            output_confs_flatten, dim=1, index=idx.expand(-1, -1, 1)
        )  # (b * h * w, occ, 1)
        ...
        if occ == 2:
            ...
            if torch.any(two_points_mask):
                ...
                two_points_color_loss = F.mse_loss(
                    input_colors_flatten[two_points_mask], output_colors_flatten[two_points_mask]
                )
                ...
                two_points_dist_loss = two_points_color_loss = two_points_conf_loss = (
                    0 * torch.mean(output_points_occ)
                )
            ...
            if torch.any(one_point_mask):
                ...
                one_point_conf_loss = F.binary_cross_entropy_with_logits(
                    ...
                    input_masks_flatten[
                        one_point_mask
                    ].float(),  # make the second point disappear (Not because there was no GT but for general regularization)
                )
            else:
                one_point_dist_loss = one_point_color_loss = one_point_conf_loss = 0 * torch.mean(
                    output_points_occ
                )
            ...
            if torch.any(zero_point_mask):
                zero_point_conf_loss = F.binary_cross_entropy_with_logits(
                    ...
                    input_masks_flatten[
                        zero_point_mask
                    ].float(),  # make the two points disappear (Not because there was no GT but for general regularization)
                )
            else:
